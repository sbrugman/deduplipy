...
def test_hierarchical_clustering_base():
    col_names = ["name"]
    scored_pairs_table = pd.read_csv(
        os.path.join("tests", "test_clustering", "clustering_fixture.csv")
    )
    res = hierarchical_clustering(scored_pairs_table, col_names)
    expected = pd.DataFrame(
        {"deduplication_id": [1, 1, 2, 3, 4], "row_number": [0, 1, 2, 3, 4]}
    )
...
def test_hierarchical_clustering_cluster_threshold():
    ...
    expected = pd.DataFrame(
        {"deduplication_id": [1, 1, 2, 2, 3], "row_number": [0, 1, 2, 3, 4]}
    )
    pd.testing.assert_frame_equal(res, expected)
    assert True
